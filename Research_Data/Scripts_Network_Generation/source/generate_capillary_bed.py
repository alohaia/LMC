...
def create_stacked_hex_network(x_min, x_max, y_min, y_max, z_min, z_max, dist_between_nodes, l_vessel, d_vessel, perturb_vs_frac=0.):
    ...
    type = np.ones(nr_of_es, dtype=np.int) * 4             # 0-PA; 1-PV; 2-DA; 3-AV; 4-C
...
def __get_2d_honeycomb(dx_box, dy_box, l_doub_edge):
    ...
    nr_of_cells_x = np.int(np.ceil(dx_box / dx_cell))
    nr_of_cells_y = np.int(np.ceil(dy_box / dy_cell))
    ...
    adj_edgelist_hexa = np.ones((nr_of_edges, 2), dtype=np.int) * (-1)
...
def __stack_honeycomb(x_caps_vs_splited, y_caps_vs_splited, adj_edgelist_splited,is_split_node, l_edge, dz_box):
    ...
    dz_cell = l_edge
    nr_of_layers = np.int(np.ceil(dz_box / dz_cell))

    split_nodes = node_ids[np.in1d(is_split_node, 1)]
    ...
    vertical_connection_nodes_layer_II = split_nodes[np.logical_not(np.in1d(split_nodes, vertical_connection_nodes_layer_I))]
    ...
    for current_layer in np.arange((nr_of_layers - 1)) + 1:
        ...
        if current_layer % 2 == 0:
            ...
            edge_list_vertical = -np.ones((nr_of_vertical_edges_current, 2), dtype=np.int)

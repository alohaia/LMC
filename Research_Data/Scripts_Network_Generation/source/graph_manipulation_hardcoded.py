...
def change_graph_attributes_casespecific_c57bl6_1(graph):
    ...
    vRBC_pre_exp[np.in1d(index_exp, 23)] *= (-1)
    vRBC_pre_exp[np.in1d(index_exp, 12)] *= (-1)
    vRBC_post_exp[np.in1d(index_exp, 23)] *= (-1)
    vRBC_post_exp[np.in1d(index_exp, 12)] *= (-1)
...
def change_graph_attributes_casespecific_c57bl6_2(graph):
    ...
    aca_in = np.zeros(graph.vcount(), dtype=np.int)
    aca_in[np.array([11,43])] = 1
    del graph.vs['ACA_in']
    graph.vs['ACA_in'] = aca_in

    mca_in = np.zeros(graph.vcount(), dtype=np.int)

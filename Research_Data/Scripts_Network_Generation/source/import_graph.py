...
def import_sa_graph_from_pkl(path_es_dict, path_vs_dict, adj_attr_name ='connectivity', graph_name='tmp', delete_tortiosity = True):
    ...
    g.es['type'] = np.zeros(g.ecount(), dtype=np.int)     # 0-PA; 1-PV; 2-DA; 3-AV; 4-C

    is_aca_in = np.in1d(np.array(g.vs['ACA_in']),1)
    is_mca_in = np.in1d(np.array(g.vs['MCA_in']),1)
    is_degree1 = np.in1d(np.array(g.vs['degree']),1)

    # fix an issue, where None was used in Attribute ACA_in and MCA_in, instead of 0
    del g.vs['ACA_in'], g.vs['MCA_in']
    aca_in = np.zeros(g.vcount(), dtype=np.int)
    aca_in[is_aca_in] = 1
    mca_in = np.zeros(g.vcount(), dtype=np.int)
    mca_in[is_mca_in] = 1
    g.vs['ACA_in'] = aca_in
    g.vs['MCA_in'] = mca_in
    ...
    DA_startingPt = np.zeros(g.vcount(),dtype=np.int)
    DA_startingPt[is_DA_startingPt] = 1

    g.vs['is_DA_startingPt'] = DA_startingPt
    g.vs['is_DA_startingPt_added_manually'] = np.zeros(g.vcount(),dtype=np.int)
    g.vs['is_AV_root'] = np.zeros(g.vcount(), dtype=np.int)
    g.vs['is_connected_2caps'] = np.zeros(g.vcount(), dtype=np.int)
    g.vs['COW_in'] = np.zeros(g.vcount(),dtype=np.int)

    g.es['is_stroke'] = np.zeros(g.ecount(), dtype=np.int)
...
def import_da_av_graph_from_pkl(path_es_dict, path_vs_dict, type, l_scaling = 1., min_diam = 6., rotation = 0,
    ...
    for e_id in xrange(g.ecount()):
        nr_subsegments = np.size(g.es['points'][e_id],0) - 1
        for sub_edge in xrange(nr_subsegments):
    ...
    g.es['length'] = length
    g.es['is_stroke'] = np.zeros(g.ecount(), dtype=np.int)
    g.es['type'] = np.ones(g.ecount(), dtype=np.int) * type  # 0-PA; 1-PV; 2-DA; 3-AV; 4-C
    ...
    if type == 2:
        ...
        is_connected_to_caps = np.logical_and(np.in1d(degree, 1),
                                              np.logical_not(np.in1d(np.array(g.vs['is_connected_to_PA']), 1)))

        is_da_root_vs = np.in1d(np.array(g.vs['is_connected_to_PA']), 1)
        ...
        is_connected_to_caps = np.logical_and(np.in1d(degree, 1),
                                              np.logical_not(np.in1d(np.array(g.vs['is_connected_to_PV']), 1)))

        is_av_outflow_vs = np.in1d(np.array(g.vs['is_connected_to_PV']), 1)
    ...
    connected_to_caps = np.zeros(g.vcount(),dtype=np.int)

...
from .import_graph import import_av_graph_from_pkl, import_da_graph_from_pkl
...
def connect_new_DA_starting_pt_to_graph(graph, xy_new_DA, target_new_min_splitedgelength = 100, max_distance_new_DA = 1000., diameter_new_vessel = -1., distort = False, distort_max = 0.):
    ...
    nr_new_subpts_ab = np.floor(length_ab / target_new_min_splitedgelength).astype(int) - 1
    ...
    eids_all_subpts = np.hstack(
        [np.ones(current_nr_subpt, dtype=int)*edge_id for current_nr_subpt, edge_id in zip(nr_new_subpts_ab, np.arange(nr_of_es_old))])
...
def merge_sa_with_trees(g_main, g_daughter, type_daughter):
    ...
    # Read PA data
    adj_list_pa = np.array(g_main.get_edgelist(), dtype=int)
    ...
    adj_list_tree = np.array(g_daughter.get_edgelist(), dtype=int) + nr_pa_vs
    ...
    vs_merged_attr['ACA_in'] = np.append(g_main.vs['ACA_in'], np.zeros(n_tree_vs, dtype=int))
    vs_merged_attr['MCA_in'] = np.append(g_main.vs['MCA_in'], np.zeros(n_tree_vs, dtype=int))
    vs_merged_attr['COW_in'] = np.append(g_main.vs['COW_in'], np.zeros(n_tree_vs, dtype=int))
    vs_merged_attr['is_DA_startingPt'] = np.append(g_main.vs['is_DA_startingPt'], np.zeros(n_tree_vs, dtype=int))
    vs_merged_attr['is_DA_startingPt_added_manually'] = np.append(g_main.vs['is_DA_startingPt_added_manually'],np.zeros(n_tree_vs, dtype=int))
    vs_merged_attr['is_AV_root'] = np.append(g_main.vs['is_AV_root'], np.zeros(n_tree_vs, dtype=int))
    ...
    vs_merged_attr['is_tree_startingPt_free'] = np.append(g_main.vs['is_tree_startingPt_free'], np.zeros(n_tree_vs, dtype=int))
    ...
    es_merged_attr['vRBC_pre_larger10'] = np.append(g_main.es['vRBC_pre_larger10'], np.zeros(n_tree_es, dtype=int))
    es_merged_attr['is_collateral'] = np.append(g_main.es['is_collateral'], np.zeros(n_tree_es, dtype=int))
    es_merged_attr['index_exp'] = np.append(g_main.es['index_exp'], -np.ones(n_tree_es, dtype=int))
    es_merged_attr['is_stroke'] = np.append(g_main.es['is_stroke'], g_daughter.es['is_stroke'])
    es_merged_attr['added_manually'] = np.append(g_main.es['added_manually'], np.ones(n_tree_es, dtype=int))
...
def add_capillary_bed(g_parent, z_min_caps=25., frame_bounding_box=50., distance_between_cap_vs=45., l_vessel=62.,
    ...
    is_tree_connect_to_caps = np.isin(np.array(g_parent.vs['is_connected_2caps']), 1)
...
def merge_with_capbed(g_main, g_daughter):
    ...
    # Read main graph
    adj_list_main = np.array(g_main.get_edgelist(), dtype=int)
    nr_vs_main = g_main.vcount()
    coords_tree_nw = np.array(g_main.vs['coords'])

    # Read cap bed
    adj_list_caps = np.array(g_daughter.get_edgelist(), dtype=int) + nr_vs_main
    ...
    vs_merged_attr['coords'] = coords_merged
    vs_merged_attr['ACA_in'] = np.append(g_main.vs['ACA_in'], np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['MCA_in'] = np.append(g_main.vs['MCA_in'], np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['COW_in'] = np.append(g_main.vs['COW_in'], np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['is_DA_startingPt'] = np.append(g_main.vs['is_DA_startingPt'], np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['is_DA_startingPt_added_manually'] = np.append(g_main.vs['is_DA_startingPt_added_manually'],
                                                                  np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['is_AV_root'] = np.append(g_main.vs['is_AV_root'], np.zeros(n_vs_caps, dtype=int))
    vs_merged_attr['is_connected_2caps'] = np.append(g_main.vs['is_connected_2caps'], np.zeros(n_vs_caps, dtype=int))
    ...
    es_merged_attr['vRBC_pre_larger10'] = np.append(g_main.es['vRBC_pre_larger10'], np.zeros(n_es_caps, dtype=int))
    es_merged_attr['is_collateral'] = np.append(g_main.es['is_collateral'], np.zeros(n_es_caps, dtype=int))
    es_merged_attr['index_exp'] = np.append(g_main.es['index_exp'], -np.ones(n_es_caps, dtype=int))
    es_merged_attr['is_stroke'] = np.append(g_main.es['is_stroke'], np.zeros(n_es_caps, dtype=int))
    es_merged_attr['added_manually'] = np.append(g_main.es['added_manually'], np.ones(n_es_caps, dtype=int))
    ...
    while nr_of_remaining_connections>0:
        ...
        adj_list_merged = np.array(g_merged.get_edgelist(), dtype=int)
        ...
        distance[np.logical_not(np.isin(type,4))] = 10*np.max(distance) # only include capillaries as connection canditates
...
def remove_regions_outside_DAs(graph, max_distance_to_da):
    ...
    nkind = np.ones(graph.vcount(), dtype=int) * 4

    vs_indices_all = np.arange(graph.vcount())
    adjacency_list = np.array(graph.get_edgelist(), dtype=int)
    vessel_type = np.array(graph.es['type'])

    is_AV = np.isin(vessel_type, 3)
    nodes_AV = np.unique(adjacency_list[is_AV])
    is_node_av = np.isin(vs_indices_all, nodes_AV)
    nkind[is_node_av] = 3

    is_PA = np.logical_or(np.isin(vessel_type, 0), np.isin(vessel_type, -1))
    nodes_PA = np.unique(adjacency_list[is_PA])
    is_node_pa = np.isin(vs_indices_all, nodes_PA)
    nkind[is_node_pa] = 0

    is_DA = np.isin(vessel_type, 2)
    nodes_DA = np.unique(adjacency_list[is_DA])
    is_node_da = np.isin(vs_indices_all, nodes_DA)
    nkind[is_node_da] = 2
